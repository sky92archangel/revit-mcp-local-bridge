...
{
    /// <summary>
    /// 所�?Revit 元素创建操作的实现�?    /// Implementation of all Revit element creation operations.
    /// </summary>
    internal static class RevitPlanCreations
    {
        /// <summary>
        /// 创建标高。校验名称唯一性并支持预览模式�?        /// Create a level. Validates name uniqueness and supports preview mode.
        /// </summary>
        public static Dictionary<string, object> CreateLevel(PlanStep step, PlanExecutionContext context)
        {
            // 从参数中获取标高值（毫米�?            double elevationMm = PlanValues.RequireMillimeters(step.Arguments, "elevation_mm", "elevation");
            ...
            // 预览模式：返回参数但不执行创�?            if (context.Preview)
            {
                return data;
            }
        ...
        /// <summary>
        /// 创建直线轴网。校验起�?终点不重合且 Z 值相同�?        /// Create a straight grid line. Validates start/end are not coincident and share the same Z value.
        /// </summary>
        public static Dictionary<string, object> CreateGrid(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("直线轴网�?start.z �?end.z 必须相同�?);
            }
            var data = new Dictionary<string, object>
            {
                { "name", name },
        ...
        /// <summary>
        /// 创建墙体。支持指定厚度自动生成新墙类型�?        /// Create a wall. Supports auto-generating a new wall type for a specified thickness.
        /// </summary>
        public static Dictionary<string, object> CreateWall(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("create_wall 目前只支�?Basic WallType�?);
            }
            ...
            // 如果指定了厚度，生成目标类型名称：前缀 RCB_ + 源类�?+ 厚度
            ...
            // 如果需要特定厚度，创建或复用对应的墙类�?            WallType targetType = requestedThicknessMm > 0.0
        ...
        /// <summary>
        /// 创建楼板。通过闭合轮廓生成楼板几何�?        /// Create a floor. Generates floor geometry from a closed boundary profile.
        /// </summary>
        ...
        /// <summary>
        /// 创建房间（基�?Revit 的房间边界放置）�?        /// Create a room (placed within Revit room-bounded areas).
        /// </summary>
        ...
        /// <summary>
        /// 创建空间（MEP 空间，类似于房间但用于暖通分析）�?        /// Create a space (MEP space, similar to room but for HVAC analysis).
        /// </summary>
        ...
        /// <summary>
        /// 创建模型线。自动计算草图平面方向�?        /// Create a model curve. Auto-computes the sketch plane orientation.
        /// </summary>
        public static Dictionary<string, object> CreateModelCurve(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (start.DistanceTo(end) < 1e-8)
            {
                throw new BridgeCommandException("模型�?start �?end 不能重合�?);
            }
            string name = PlanValues.String(step.Arguments, null, "name");
            var data = new Dictionary<string, object>
            {
            ...
            // 计算法向量以创建草图平面：优先使�?Z 轴，平行时改�?X �?            XYZ direction = (end - start).Normalize();
        ...
        /// <summary>
        /// 创建视图�?D、平面、天花板、结构平面）。支持相机方位设置�?        /// Create a view (3D, floor plan, ceiling plan, structural plan). Supports camera orientation.
        /// </summary>
        public static Dictionary<string, object> CreateView(PlanStep step, PlanExecutionContext context)
        {
            ...
            switch (kind)
            {
                ...
                case "structural_plan":
                    ...
                    break;
                default:
                    throw new BridgeCommandException("create_view.kind 仅支�?3d、floor_plan、ceiling_plan、structural_plan�?);
            }
            ...
            XYZ forward = null;
            XYZ up = null;
            // 3D 视图可读取相机参数：eye（视点）、forward（视线方向）、up（上方向�?            if (family == ViewFamily.ThreeDimensional && PlanValues.Get(step.Arguments, "eye", "eye_position", "camera_position") != null)
            {
                ...
                {
                    throw new BridgeCommandException("create_view �?forward �?up 必须近似垂直�?);
                }
            }
            var data = new Dictionary<string, object>
            {
            ...
            else
            {
                if (perspective)
                {
                    throw new BridgeCommandException("perspective=true 仅适用�?3d 视图�?);
                }
            ...
            if (eye != null)
            {
                ...
                if (view3D == null)
                {
                    throw new BridgeCommandException("相机参数仅适用�?3d 视图�?);
                }
        ...
        /// <summary>
        /// 创建图纸。可指定图框类型和编�?名称�?        /// Create a sheet. Optionally specify a title block type and sheet number/name.
        /// </summary>
        ...
        /// <summary>
        /// 将视图放置到图纸上。校验视图是否可放置�?        /// Place a view onto a sheet. Validates whether the view can be placed.
        /// </summary>
        public static Dictionary<string, object> PlaceViewOnSheet(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("该视图不能放置到指定图纸；它可能已被放置，或属于明细�?图例等特殊视图�?);
            }
            if (context.Preview)
            {
                return data;
        ...
        /// <summary>
        /// 创建洞口（墙洞、竖直楼板洞、竖井洞）�?        /// Create an opening (wall opening, vertical floor opening, shaft opening).
        /// </summary>
        public static Dictionary<string, object> CreateOpening(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("create_opening.kind 仅支�?wall、vertical、shaft�?);
            }
            if (kind == "shaft")
            {
            ...
            if (kind == "wall")
            {
                ...
                if (wall == null)
                {
                    throw new BridgeCommandException("create_opening(kind=wall) �?host_id 必须指向墙体�?);
                }
                XYZ start = PlanValues.Point(step.Arguments, "start");
                XYZ end = PlanValues.Point(step.Arguments, "end");
                if (start.DistanceTo(end) < 1e-8)
                {
                    throw new BridgeCommandException("洞口 start �?end 不能重合�?);
                }
            ...
            if (floor == null)
            {
                throw new BridgeCommandException("create_opening(kind=vertical) �?host_id 必须指向楼板�?);
            }
            // 兼容 corner_1/corner_2 �?start/end 参数�?            XYZ corner1 = PlanValues.Point(step.Arguments,
            ...
            {
                throw new BridgeCommandException("竖直洞口 corner_1 �?corner_2 �?X、Y 分量均不能相同�?);
            }
        ...
        /// <summary>
        /// 创建竖井洞口（从底部标高到顶部标高）�?        /// Create a shaft opening spanning from bottom level to top level.
        /// </summary>
        ...
        /// <summary>
        /// 从参数字段解析标高（�?ID 或名称）�?        /// Resolve a level from arguments by ID or name field.
        /// </summary>
        ...
        /// <summary>
        /// 从点列表构建闭合边界环（用于竖井等）�?        /// Build a closed boundary loop from a list of points (for shaft openings, etc.).
        /// </summary>
        private static CurveLoop BuildBoundaryLoop(IDictionary<string, object> arguments, string fieldName)
        {
            ...
            if (points.Count < 3)
            {
                throw new BridgeCommandException(fieldName + " 至少需�?3 个点�?);
            }
        ...
        /// <summary>
        /// 构建闭合轮廓线集（用于楼板等），基于指定基础标高�?        /// Build a closed CurveArray profile for floors etc., offset from a base elevation.
        /// </summary>
            ...
            string operation)
        {
            ...
            if (points.Count < 3)
            {
                throw new BridgeCommandException(operation + ".boundary 至少需�?3 个点�?);
            }
            var resolved = new List<XYZ>();
            foreach (Dictionary<string, object> point in points)
            {
        ...
        /// <summary>
        /// 解析视图族类型（ViewFamilyType），�?ID 或名称查找�?        /// Resolve a ViewFamilyType by ID or name, matching the expected view family.
        /// </summary>
        ...
        /// <summary>
        /// 解析可选的图框族符号。如未指定任何参数则返回 null�?        /// Resolve an optional title block family symbol. Returns null if no arguments specified.
        /// </summary>
            ...
            IDictionary<string, object> arguments)
        {
            ...
            {
                throw new BridgeCommandException("指定�?title_block 不是图框族类型�?);
            }
            return symbol;
        }

        /// <summary>
        /// 创建 DirectShape（直接形状），支持材质和几何描述�?        /// Create a DirectShape element with optional material and geometry description.
        /// </summary>
        public static Dictionary<string, object> CreateDirectShape(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("DirectShape 不支持类�?ID�? + categoryId.GetValue());
            }
            ...
            {
                throw new BridgeCommandException("找不到材质\u201c" + materialName + "\u201d。设�?create_material_if_missing=true 可在执行时创建�?);
            }
            ...
            {
                throw new BridgeCommandException("生成�?DirectShape 几何无效，未写入模型�?);
            }
        ...
        /// <summary>
        /// 创建 MEP 曲线（管道、风管、线管、桥架）。支持坡度和尺寸设置�?        /// Create MEP curves (pipe, duct, conduit, cable tray). Supports slope and sizing.
        /// </summary>
        public static Dictionary<string, object> CreateMepCurve(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (start.DistanceTo(end) < 1e-8)
            {
                throw new BridgeCommandException("机电曲线 start �?end 不能重合�?);
            }
            // 坡度计算：根据起点终点水平距离调整终�?Z �?            object rawSlope = PlanValues.Get(step.Arguments, "slope", "slope_percent", "slope_permille");
            if (rawSlope != null)
            {
            ...
            switch (normalizedKind)
            {
                ...
                case "cabletray":
                    ...
                    break;
                default:
                    throw new BridgeCommandException("create_mep_curve.kind 仅支�?pipe、duct、conduit、cable_tray�?);
            }
        ...
        /// <summary>
        /// 连接两个 MEP 元素（管道、风管等），支持多种连接件类型�?        /// Connect two MEP elements (pipes, ducts, etc.) with various fitting types.
        /// </summary>
        public static Dictionary<string, object> ConnectMep(PlanStep step, PlanExecutionContext context)
        {
            ...
            // 可选：延伸管线至交点后再连�?            if (PlanValues.Boolean(step.Arguments, false, "extend_to_intersection", "extend"))
            {
            ...
            // 根据 fitting 类型创建对应连接�?            switch (fitting)
            {
                case "auto":
                case "direct":
                ...
                case "cross":
                    ...
                    break;
                default:
                    throw new BridgeCommandException("connect_mep.fitting 仅支�?auto、direct、elbow、union、tee、reducer、cross�?);
            }
        ...
        /// <summary>
        /// 创建 MEP 系统（管道系统或机械系统），并可添加成员�?        /// Create an MEP system (piping or mechanical) and optionally add members.
        /// </summary>
        public static Dictionary<string, object> CreateMepSystem(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("create_mep_system.domain 仅支�?piping、mechanical�?);
            }
            ...
            if (rawMembers != null)
            {
                ...
                {
                    if (memberId != ElementId.InvalidElementId)
                    {
            ...
            // 将成员逐一添加到系统中（通过连接件集�?            int added = 0;
        ...
        /// <summary>
        /// 加载族文件（.rfa）到当前项目，可选激活特定族类型�?        /// Load a family file (.rfa) into the current project, optionally activating a specific symbol.
        /// </summary>
        ...
        /// <summary>
        /// 创建管道/风管保温层�?        /// Create pipe or duct insulation.
        /// </summary>
        ...
        /// <summary>
        /// 解析保温层类�?ID（PipeInsulationType �?DuctInsulationType）�?        /// Resolve the insulation type ID (PipeInsulationType or DuctInsulationType).
        /// </summary>
        private static ElementId ResolveInsulationTypeId<T>(Document document, string typeName) where T : ElementType
        {
            ...
            {
                throw new BridgeCommandException(
                    "当前项目没有可用 " + typeof(T).Name + (string.IsNullOrWhiteSpace(typeName) ? string.Empty : "（名�?" + typeName + "�?) + "�?);
            }
            return candidates[0].Id;
        }

        /// <summary>
        /// 创建放样形状（扫掠体）。支持矩�?圆形截面，可含壁厚�?        /// Create a swept solid shape. Supports rectangular/circular profiles with optional wall thickness.
        /// </summary>
        ...
        /// <summary>
        /// 放置族实例。支持多种放置类型（基于标高、基于面、工作平面、视图等）�?        /// Place a family instance. Supports multiple placement types (level-based, face-hosted, work-plane, view-based, etc.).
        /// </summary>
        public static Dictionary<string, object> PlaceFamilyInstance(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("族类型缺�?Family 信息�?);
            }
            ...
            switch (placementType)
            {
                ...
                case FamilyPlacementType.WorkPlaneBased:
                    ...
                    if (useHostFace)
                    {
                        ...
                        if (faceHostId == ElementId.InvalidElementId)
                        {
                            return DeferredPlacement(data, "$host");
                        }
                ...
                case FamilyPlacementType.CurveDrivenStructural:
                    ...
                    if (start.DistanceTo(end) < 1e-8)
                    {
                        throw new BridgeCommandException("线基�?start �?end 不能重合�?);
                    }
                ...
                case FamilyPlacementType.Adaptive:
                    ...
                    {
                        throw new BridgeCommandException(
                            "自适应族需�?" + pointIds.Count + " �?adaptive_points，当前收�?" + adaptivePoints.Count + " 个�?);
                    }
        ...
        /// <summary>
        /// 创建结构构件（梁、支撑、柱）�?        /// Create structural members (beam, brace, column).
        /// </summary>
        public static Dictionary<string, object> CreateStructuralMember(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (normalizedKind == "beam" || normalizedKind == "brace")
            {
                ...
                if (start.DistanceTo(end) < 1e-8)
                {
                    throw new BridgeCommandException("结构线构�?start �?end 不能重合�?);
                }
            ...
            throw new BridgeCommandException("create_structural_member.kind 仅支�?beam、brace、column�?);
        }

        /// <summary>
        /// 解析楼板类型。过滤掉基础底板�?        /// Resolve a FloorType, filtering out foundation slabs.
        /// </summary>
        ...
        /// <summary>
        /// 解析系统类型（如 PipingSystemType / MechanicalSystemType）�?        /// Resolve a system type (e.g. PipingSystemType / MechanicalSystemType).
        /// </summary>
            ...
            where T : ElementType
        {
            ...
            if (candidates.Count == 0)
            {
                throw new BridgeCommandException("当前项目没有可用 " + typeof(T).Name + "。先�?query_catalog(kind=mep_types) 查询�?);
            }
            return candidates[0];
        }

        /// <summary>
        /// 在预览数据中添加 MEP 尺寸描述（直�?�?高）�?        /// Add MEP size description (diameter/width/height) to preview data.
        /// </summary>
            ...
            bool allowRectangular)
        {
            ...
            if (diameter != null)
            {
                if (!allowDiameter)
                {
                    throw new BridgeCommandException("�?MEP 曲线不支�?diameter_mm�?);
                }
            ...
            if (width != null || height != null)
            {
                if (!allowRectangular)
                {
                    throw new BridgeCommandException("�?MEP 曲线不支�?width_mm/height_mm�?);
                }
                if (width != null)
                {
        ...
        /// <summary>
        /// 在已创建�?MEP 元素上设置尺寸（直径/�?高）�?        /// Set sizing (diameter/width/height) on a created MEP element.
        /// </summary>
        private static void SetMepSize(Element element, IDictionary<string, object> arguments, bool allowDiameter, bool allowRectangular)
        {
            ...
            if (PlanValues.Get(arguments, "diameter_mm", "diameter") != null && allowDiameter)
            {
                ...
                {
                    throw new BridgeCommandException("创建�?MEP 曲线不支持设置直径参数�?);
                }
            }
            if (allowRectangular)
            {
                ...
                {
                    throw new BridgeCommandException("创建�?MEP 曲线不支持设置宽度参数�?);
                }
                ...
                {
                    throw new BridgeCommandException("创建�?MEP 曲线不支持设置高度参数�?);
                }
            }
        }

        /// <summary>
        /// 尝试设置元素的可选长度参数（毫米值转 Revit 内部单位）�?        /// Attempt to set an optional length parameter (mm value converted to Revit internal units).
        /// </summary>
        ...
        /// <summary>
        /// 获取文档中指�?ID 的元素，不存在时抛出异常�?        /// Get an element by ID from the document, throwing if not found.
        /// </summary>
        private static Element RequireElement(Document document, ElementId id, string fieldName)
        {
            ...
            if (element == null)
            {
                throw new BridgeCommandException("找不�?" + fieldName + "=" + id.GetValue() + " 对应元素�?);
            }
            return element;
        }

        /// <summary>
        /// 查找 MEP 元素的连接器（按索引或最近连接器）�?        /// Find a connector on an MEP element (by index or nearest to another element's connectors).
        /// </summary>
            ...
            Element otherElement)
        {
            ...
            // 未指定索引时，选择距离另一元素最近的连接�?            List<Connector> otherConnectors = GetConnectors(otherElement);
        ...
        /// <summary>
        /// 延伸两条 MEP 曲线至其交点（直线段适用）�?        /// Extend two MEP curves to their intersection point (for straight segments only).
        /// </summary>
        private static XYZ ExtendMepCurvesToIntersection(Document document, Element first, Element second)
        {
            ...
            {
                throw new BridgeCommandException("extend_to_intersection 仅支持管�?/ 风管 / 线管 / 桥架�?MEP 曲线�?);
            }
        ...
        /// <summary>
        /// 将直线修剪到指定点（保留距该点较近的端点到该点之间的线段）�?        /// Trim a line to a given point (keeps the segment from the nearer endpoint to the point).
        /// </summary>
        ...
        /// <summary>
        /// 获取 MEP 元素的端点连接器列表�?        /// Get the list of end-type connectors for an MEP element.
        /// </summary>
        ...
        /// <summary>
        /// 解析结构类型字符串到 StructuralType 枚举�?        /// Parse a structural type string to the StructuralType enum.
        /// </summary>
        ...
        /// <summary>
        /// 返回延迟放置结果（预览时前置引用尚未就绪）�?        /// Return a deferred placement result (preceding reference not yet available in preview).
        /// </summary>
        ...
        {
            data["deferred"] = true;
            data["reason"] = "preview 中前置元素引用尚无真�?ID�? + reference;
            return data;
        }

        /// <summary>
        /// 解析目标视图（必须是有效非样板视图）�?        /// Resolve a target view (must be a valid non-template view).
        /// </summary>
            ...
            params string[] names)
        {
            ...
            {
                throw new BridgeCommandException("预览中视图引用尚无真�?ID。请将该步骤单独预览，或直接指定已有 view_id�?);
            }
        ...
        /// <summary>
        /// 解析或创建工作平面。可复用已有工作平面或根据法向量创建新的�?        /// Resolve or create a sketch plane. Reuses an existing one or creates from a normal vector.
        /// </summary>
        ...
        /// <summary>
        /// 读取可选点参数�?        /// Read an optional point parameter.
        /// </summary>
        ...
        /// <summary>
        /// 读取方向向量参数并归一化�?        /// Read a direction vector parameter and normalize it.
        /// </summary>
        ...
        /// <summary>
        /// 读取参考方向（放置族时的方向参数）�?        /// Read the reference direction parameter (for family instance placement).
        /// </summary>
        ...
        /// <summary>
        /// 查找宿主元素上指定索引的面引用�?        /// Find a face reference on a host element by index.
        /// </summary>
        private static Reference FindFaceReference(Element host, int requestedIndex)
        {
            if (requestedIndex < 0)
            {
                throw new BridgeCommandException("host_face_index 必须大于或等�?0�?);
            }
            ...
            {
                throw new BridgeCommandException(
                    "host_face_index=" + requestedIndex + " 超出宿主可引用面的数�?" + references.Count + "�?);
            }
            return references[requestedIndex];
        }

        /// <summary>
        /// 递归收集几何中的面引用�?        /// Recursively collect face references from geometry.
        /// </summary>
        ...
        /// <summary>
        /// 读取自适应点列表�?        /// Read a list of adaptive placement points.
        /// </summary>
        private static List<XYZ> ReadAdaptivePoints(IDictionary<string, object> arguments)
        {
            ...
            if (raw.Count == 0)
            {
                throw new BridgeCommandException("自适应族需�?adaptive_points 数组�?);
            }
        ...
        /// <summary>
        /// 激活族类型（如果尚未激活）�?        /// Activate a family symbol if it is not already active.
        /// </summary>
        ...
        /// <summary>
        /// 解析或创建墙类型。如指定厚度的类型已存在则复用，否则复制源类型并调整厚度�?        /// Resolve or create a wall type. Reuses an existing type at the target thickness or duplicates and adjusts.
        /// </summary>
