namespace RevitCommandBridge
{
    ...
    internal static class BridgeFileQueue
    {
        ...
        public static string Enqueue(BridgeRequest request)
        {
            ...
            lock (FileGate)
            {
                // 检�?ID 是否已存在于 inbox / processing / outbox �?
                ...
                {
                    throw new BridgeCommandException("命令 ID 已存在：" + request.Id + "。请生成�?ID，或读取已有结果�?);
                }
        ...
        /// <summary>
        /// 恢复上次中断时遗留在 processing 目录中的请求（移�?inbox）�?
        ...
        /// <summary>
        /// 追加日志�?bridge.log。异常静默处理以免影�?Revit 命令执行�?
        ...
        private static string NormalizeRequestId(string id)
        {
            ...
            {
                throw new BridgeCommandException("命令 ID 必须�?1 �?128 个字符�?);
            }
