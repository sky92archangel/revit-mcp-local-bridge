namespace RevitCommandBridge
{
    ...
    internal static class RevitPlanMutations
    {
        ...
        /// <param name="context">执行上下�?/ The execution context.</param>
        public static Dictionary<string, object> SetParameters(PlanStep step, PlanExecutionContext context)
        {
            ...
            foreach (ElementId id in ids)
            {
                ...
                foreach (KeyValuePair<string, object> pair in requested)
                {
                    ...
                    if (parameter == null)
                    {
                        ...
                        throw new BridgeCommandException("元素 " + id.GetValue() + " 找不到参数�? + pair.Key + "”�?);
                    }
                    if (parameter.IsReadOnly)
                    {
                        ...
                        throw new BridgeCommandException("元素 " + id.GetValue() + " 的参数�? + pair.Key + "”是只读�?);
                    }
            ...
            {
                { "element_ids", ids.Select(id => id.GetValue()).ToArray() },
                { "changes", changes },
        ...
        public static Dictionary<string, object> DeleteElements(PlanStep step, PlanExecutionContext context)
        {
            ...
            foreach (ElementId id in ids)
            {
                ...
                {
                    throw new BridgeCommandException("找不到待删除 element_id=" + id.GetValue() + "�?);
                }
            }
            var data = new Dictionary<string, object>
            {
                { "requested_element_ids", ids.Select(id => id.GetValue()).ToArray() }
            };
            if (context.Preview)
            {
                return data;
            ...
            data["deleted_element_ids"] = deleted.Select(id => id.GetValue()).ToArray();
            return data;
        }
        ...
        public static Dictionary<string, object> SelectElements(PlanStep step, PlanExecutionContext context)
        {
            ...
            foreach (ElementId id in ids)
            {
                if (context.Document.GetElement(id) == null)
                {
                    throw new BridgeCommandException("找不到待选择 element_id=" + id.GetValue() + "�?);
                }
            }
        ...
        public static Dictionary<string, object> DuplicateType(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (sourceId == ElementId.InvalidElementId)
            {
                return new Dictionary<string, object> { { "deferred", true }, { "reason", "preview 中前置类型引用尚无真�?ID�? } };
            }
            ElementType sourceType = context.Document.GetElement(sourceId) as ElementType;
            if (sourceType == null)
            {
                throw new BridgeCommandException("duplicate_type �?type_id 必须指向 ElementType（类型）�?);
            }
            ...
            var data = new Dictionary<string, object>
            {
                { "source_type_id", sourceId.GetValue() },
            ...
            foreach (KeyValuePair<string, object> pair in requested)
            {
                ...
                if (parameter == null)
                {
                    throw new BridgeCommandException("类型 " + sourceId.GetValue() + " 找不到参数�? + pair.Key + "”�?);
                }
                if (parameter.IsReadOnly)
                {
                    throw new BridgeCommandException("类型 " + sourceId.GetValue() + " 的参数�? + pair.Key + "”是只读�?);
                }
                ValidateParameterValue(parameter, pair.Value, pair.Key);
            }
            if (context.Preview)
            ...
            data["element_id"] = newTypeId.GetValue();
            data["element_ids"] = new[] { newTypeId.GetValue() };
        ...
        public static Dictionary<string, object> ManageSchemaData(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("manage_schema_data.action 仅支�?set、get、clear、transport�?);
            }
            ...
            if (action == "set")
            {
                ...
                if (raw.Count == 0)
                {
                    throw new BridgeCommandException("manage_schema_data.set �?values 不能为空�?);
                }
            ...
            if (action == "transport")
            {
                ...
                if (source == null)
                {
                    throw new BridgeCommandException("找不�?source_element_id=" + sourceId.GetValue() + "�?);
                }
                values = BridgeSchemas.ReadMap(source);
                if (values == null || values.Count == 0)
                {
                    throw new BridgeCommandException("源元素没有可搬运的扩展数据：" + sourceId.GetValue());
                }
                data["values"] = values;
            }
            ...
            switch (action)
            {
                ...
                case "transport":
                    foreach (ElementId id in targets)
                    {
                        ...
                        if (element == null)
                        {
                            throw new BridgeCommandException("找不�?element_id=" + id.GetValue() + "�?);
                        }
                ...
                case "get":
                {
                    ...
                    if (element == null)
                    {
                        throw new BridgeCommandException("找不�?element_id=" + targets[0].GetValue() + "�?);
                    }
        ...
        public static Dictionary<string, object> ManageFamilyParameters(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (family == null)
            {
                throw new BridgeCommandException("manage_family_parameters �?family_id 必须指向族（Family）�?);
            }
            ...
            if (actions.Count == 0)
            {
                throw new BridgeCommandException("manage_family_parameters 至少需要一�?actions 项�?);
            }
            ...
            foreach (Dictionary<string, object> item in actions)
            {
                ...
                switch (verb)
                {
                    ...
                    case "rename":
                        ...
                        {
                            throw new BridgeCommandException("rename 动作需�?new_name�?);
                        }
                    ...
                    case "set_formula":
                        ...
                        {
                            throw new BridgeCommandException("set_formula 动作需�?formula�?);
                        }
                        described.Add("set_formula " + name);
                        break;
                    default:
                        throw new BridgeCommandException("actions[].action 仅支�?add、rename、remove、set_formula�?);
                }
            }
            var data = new Dictionary<string, object>
            {
                { "family", family.Name },
                { "family_id", familyId.GetValue() },
...
Family loadedFamilyResult;
            ...
            data["element_id"] = familyId.GetValue();
            data["element_ids"] = new[] { familyId.GetValue() };
            return data;
        }
        ...
        public static Dictionary<string, object> TransformElements(PlanStep step, PlanExecutionContext context)
        {
            ...
            foreach (ElementId id in ids)
            {
                if (context.Document.GetElement(id) == null)
                {
                    throw new BridgeCommandException("找不到待变换 element_id=" + id.GetValue() + "�?);
                }
            }
            ...
            switch (mode)
            {
                case "move":
                    ...
                    {
                        throw new BridgeCommandException("transform_elements.move �?translation 不能为零向量�?);
                    }
                ...
                case "copy":
                    ...
                    {
                        throw new BridgeCommandException("transform_elements.copy �?translation 不能为零向量�?);
                    }
                    ICollection<ElementId> copied = ElementTransformUtils.CopyElements(document, ids, offset);
                    data["copied_count"] = copied.Count;
                    data["element_ids"] = copied.Select(id => id.GetValue()).ToArray();
                    break;
                case "rotate":
                    ...
                    {
                        throw new BridgeCommandException("transform_elements.rotate �?angle 不能�?0�?);
                    }
                ...
                case "mirror":
                    ...
                    {
                        throw new BridgeCommandException("transform_elements.mirror �?plane_normal 不能为零向量�?);
                    }
                    ...
                    data["element_ids"] = mirrored.Select(id => id.GetValue()).ToArray();
                    break;
                default:
                    throw new BridgeCommandException("transform_elements.mode 仅支�?move、copy、rotate、mirror�?);
            }
            return data;
        }
        ...
        public static Dictionary<string, object> RenameElement(PlanStep step, PlanExecutionContext context)
        {
            ...
            {
                throw new BridgeCommandException("rename_element 需�?name（单目标）或 prefix（批量模式）�?);
            }
            if (!string.IsNullOrWhiteSpace(name) && !string.IsNullOrWhiteSpace(prefix))
            {
                throw new BridgeCommandException("rename_element �?name �?prefix 只能二选一�?);
            }
            if (!string.IsNullOrWhiteSpace(name) && ids.Count > 1)
            {
                throw new BridgeCommandException("rename_element.name 模式只支持单个目标；批量请使�?prefix�?);
            }
            ...
            foreach (ElementId id in ids)
            {
                ...
                if (element == null)
                {
                    throw new BridgeCommandException("找不到待重命�?element_id=" + id.GetValue() + "�?);
                }
                string oldName = RevitLookups.ElementName(element);
                string newName = string.IsNullOrWhiteSpace(prefix)
                    ? name
                    : (withIdSuffix ? prefix + id.GetValue() : prefix + oldName);
                try
                {
                ...
                catch (Exception ex)
                {
                    throw new BridgeCommandException(
                        "重命�?element_id=" + id.GetValue() + "�? + oldName + " �?" + newName + "）失败：" + ex.Message);
                }
            ...
            data["element_ids"] = ids.Select(id => id.GetValue()).ToArray();
            return data;
        }
        ...
        public static Dictionary<string, object> SetElementCurve(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (location == null)
            {
                throw new BridgeCommandException("set_element_curve 目标必须是线状图元（�?/ 管道 / 线管 / 桥架 / 模型线）�?);
            }
            XYZ start = PlanValues.Point(step.Arguments, "start");
            XYZ end = PlanValues.Point(step.Arguments, "end");
            if (start.DistanceTo(end) < 1e-8)
            {
                throw new BridgeCommandException("set_element_curve �?start �?end 不能重合�?);
            }
            var data = new Dictionary<string, object>
            {
                { "element_id", id.GetValue() },
            ...
            data["element_ids"] = new[] { id.GetValue() };
            return data;
        }
        ...
        /// <summary>
        /// 读取旋转轴方向：支持 x/y/z 字符串或 {x,y,z} 向量对象，默�?BasisZ�?
        ...
        private static XYZ ReadAxisDirection(IDictionary<string, object> arguments)
        {
            ...
            if (text != null)
            {
                switch (text.Trim().ToLowerInvariant())
                {
                    ...
                    default:
                        throw new BridgeCommandException("axis_direction 仅支�?x、y、z �?{x,y,z} 向量�?);
                }
            }
        ...
        private static double ReadDoubleValue(object raw, string name)
        {
            ...
            if (rawValue == null)
            {
                throw new BridgeCommandException("参数�? + name + "”的数值对象缺�?value�?);
            }
            ...
            switch (unit)
            {
                ...
                case "radians":
                    return value;
                default:
                    throw new BridgeCommandException("参数�? + name + "”不支持 unit=�? + unit + "”。支�?internal、mm、m、ft、deg、rad�?);
            }
        }
