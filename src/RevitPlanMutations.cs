...
namespace RevitCommandBridge
{
    ...
    internal static class RevitPlanMutations
    {
        ...
        public static Dictionary<string, object> DuplicateType(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (sourceId.GetValue() == ElementId.InvalidElementId.GetValue())
            {
        ...
        public static Dictionary<string, object> ManageSchemaData(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (action == "transport")
            {
                ...
                if (source == null)
                {
                    throw new BridgeCommandException("找不到 source_element_id=" + sourceId.GetValue() + "。");
                }
            ...
            switch (action)
            {
                ...
                case "transport":
                    foreach (ElementId id in targets)
                    {
                        ...
                        if (element == null)
                        {
                            throw new BridgeCommandException("找不到 element_id=" + id.GetValue() + "。");
                        }
                ...
                case "get":
                {
                    ...
                    if (element == null)
                    {
                        throw new BridgeCommandException("找不到 element_id=" + targets[0].GetValue() + "。");
                    }
        ...
        public static Dictionary<string, object> ManageFamilyParameters(PlanStep step, PlanExecutionContext context)
        {
            ...
            if (familyId.GetValue() == ElementId.InvalidElementId.GetValue())
            {
...
Family loadedFamilyResult;
        ...
        public static Dictionary<string, object> RenameElement(PlanStep step, PlanExecutionContext context)
        {
            ...
            foreach (ElementId id in ids)
            {
                ...
                if (element == null)
                {
                    throw new BridgeCommandException("找不到待重命名 element_id=" + id.GetValue() + "。");
                }
                ...
                catch (Exception ex)
                {
                    throw new BridgeCommandException(
                        "重命名 element_id=" + id.GetValue() + "（" + oldName + " → " + newName + "）失败：" + ex.Message);
                }
