namespace RevitCommandBridge
{
    ...
    {
        /// <summary>
        /// �?level_id、level 名称或默认（第一个）解析标高�?/ Resolves a level by level_id, level name, or defaults to the first level.
        /// </summary>
        public static Level ResolveLevel(Document document, IDictionary<string, object> arguments)
        {
            ...
            if (idValue != null)
            {
                ...
                if (byId == null)
                {
                    throw new BridgeCommandException("找不�?level_id=" + id + " 对应的标高�?);
                }
                return byId;
            }
        ...
        /// <summary>
        /// �?type_id、family/type 名称解析元素类型，支持默认值�?/ Resolves an ElementType by type_id or family/type name, with optional default fallback.
        /// </summary>
            ...
            bool allowDefault)
        {
            ...
            if (idValue != null)
            {
                ...
                {
                    throw new BridgeCommandException("type_id=" + id + " 不是可用�?" + expectedType.Name + "�?);
                }
                return byId;
            }
            ...
            if (candidates.Count == 0)
            {
                throw new BridgeCommandException(
                    "找不到匹配的 " + expectedType.Name + "。先�?query_catalog(kind=types) 查询现有类型�?);
            }
            ...
            {
                throw new BridgeCommandException("族与类型条件匹配到多个结果，请提�?type_id 精确指定�?);
            }
            return candidates[0];
        }

        /// <summary>
        /// 按参数解�?FamilySymbol�?/ Resolves a FamilySymbol from arguments.
        /// </summary>
        ...
        /// <summary>
        /// �?BuiltInCategory 枚举、数�?ID 或类别名称解析类别�?/ Resolves a category by BuiltInCategory enum, numeric ID, or category name.
        /// </summary>
            ...
            BuiltInCategory defaultCategory)
        {
            ...
            throw new BridgeCommandException("找不到类别�? + text + "”。可�?OST_*、类�?ID 或当�?Revit 的类别名称�?);
        }

        /// <summary>
        /// 按名称查找材质，未找到或名称为空时返�?null�?/ Finds a material by name; returns null if not found or name is empty.
        /// </summary>
...
data["display_unit_type"] = parameter.GetUnitTypeId().TypeId;
        ...
        /// <summary>
        /// 将值解析为正整数元�?ID�?/ Parses a value as a positive integer element ID.
        /// </summary>
