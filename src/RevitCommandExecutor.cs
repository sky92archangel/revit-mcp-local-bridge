...
{
    /// <summary>
    /// 兼容操作模式的命令执行器：处�?health、list_levels、create_wall 等单步命令�?/ Legacy command executor handling single-step operations such as health, list_levels, create_wall, etc.
    /// </summary>
    internal static class RevitCommandExecutor
    {
        ...
        /// <summary>
        /// �?operation 分派到对应的处理函数�?/ Dispatches to the appropriate handler based on the operation name.
        /// </summary>
        public static BridgeResponse Execute(UIApplication uiApplication, BridgeRequest request)
        {
            ...
            {
                throw new BridgeCommandException("请先�?Revit 中打开一个项目文档�?);
            }
            ...
            switch (operation)
            {
                ...
                default:
                    throw new BridgeCommandException(
                        "不支�?operation=�? + request.Operation + "”。支持：health、list_family_templates、create_family、load_family、execute_plan，以及兼容操�?list_levels、list_wall_types、create_level、create_grid、create_rectangle_walls、create_wall、new_project�?);
            }
        }
        ...
        private static BridgeResponse Health(UIApplication uiApplication, string expectedDocumentTitle)
        {
            ...
            if (uiApplication.ActiveUIDocument == null)
            {
                ...
                {
                    return BridgeResponse.Failure("当前没有打开项目文档，无法匹�?document_title�?, data);
                }
        ...
        /// <summary>
        /// 创建并可选地保存�?Revit 项目�?/ Creates and optionally saves a new Revit project.
        /// </summary>
        private static BridgeResponse CreateProject(UIApplication uiApplication, BridgeRequest request)
        {
            if (!string.IsNullOrWhiteSpace(request.DocumentTitle))
            {
                throw new BridgeCommandException("new_project 不接�?document_title。请在创建后使用 health 查询新文档标题�?);
            }
            ...
            if (!string.IsNullOrWhiteSpace(savePath))
            {
                ...
                {
                    throw new BridgeCommandException("new_project.save_path 必须�?.rvt 结尾�?);
                }
                if (File.Exists(normalizedSavePath) && !overwriteFile)
                {
                    throw new BridgeCommandException("目标项目文件已存在。设�?overwrite_file=true 才会覆盖�? + normalizedSavePath);
                }
        ...
        private static BridgeResponse CreateLevel(Document document, BridgeRequest request)
        {
            ...
            plan["id"] = (int)created.Id.GetValue();
        ...
        {
            double widthMm = BridgeArguments.RequireMillimeters(request, "width_mm", "width", "�?);
            double depthMm = BridgeArguments.RequireMillimeters(request, "depth_mm", "depth", "�?, "进深");
            ...
            string wallTypeName = BridgeArguments.GetString(request, null, "wall_type", "wall_type_name", "墙类�?);
            ...
            if (request.Preview)
            {
                return BridgeResponse.Success("preview", "预览：将在�? + level.Name + "”创�?4 面矩形墙�?, plan);
            }
            ...
            plan["wall_ids"] = walls.Select(wall => (int)wall.Id.GetValue()).ToArray();
            return BridgeResponse.Success("completed", "已创�?4 面矩形墙�?, plan);
        }
        ...
        private static BridgeResponse CreateWall(Document document, BridgeRequest request)
        {
            ...
            {
                throw new BridgeCommandException("墙高和墙厚必须大�?0�?);
            }
        ...
        private static WallType ResolveOrCreateWallType(Document document, WallType sourceType, string targetName, double thicknessMm)
        {
            ...
            if (existing != null)
            {
                ...
                {
                    throw new BridgeCommandException(
                        "墙类型�? + targetName + "”已存在，但厚度�?" + RoundMillimeters(existing.Width) + "mm，不会改动既有类型�?);
                }
        ...
        /// <summary>
        /// �?x/y 毫米坐标转为字典�?/ Converts x/y millimeter coordinates to a dictionary.
        /// </summary>
        ...
        /// <summary>
        /// �?x/y 毫米坐标转为 Revit XYZ（Z=0）�?/ Converts x/y millimeter coordinates to a Revit XYZ (Z=0).
        /// </summary>
        ...
        /// <summary>
        /// �?x/y 毫米坐标和标高（英尺）转�?Revit XYZ�?/ Converts x/y millimeter coordinates and elevation (feet) to a Revit XYZ.
        /// </summary>
