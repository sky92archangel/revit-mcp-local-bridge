namespace RevitCommandBridge
{
    ...
    internal static class RevitFamilyOperations
    {
        ...
        public static BridgeResponse ListFamilyTemplates(UIApplication uiApplication, BridgeRequest request)
        {
            ...
            {
                throw new BridgeCommandException("list_family_templates.limit 必须�?1 �?" + MaximumTemplateLimit + " 之间�?);
            }
            ...
            return BridgeResponse.Success("completed", "读取�?" + templates.Count + " 个族样板�?, data);
        }
        ...
        public static BridgeResponse LoadFamily(UIApplication uiApplication, Document projectDocument, BridgeRequest request)
        {
            ...
            data["family_id"] = (int)family.Id.GetValue();
            data["family_name"] = family.Name;
            data["placement_type"] = family.FamilyPlacementType.ToString();
            data["symbol_ids"] = family.GetFamilySymbolIds().Select(id => (int)id.GetValue()).ToArray();
        ...
        public static BridgeResponse CreateFamily(UIApplication uiApplication, Document projectDocument, BridgeRequest request)
        {
            ...
            {
                throw new BridgeCommandException("create_family.geometry 最多允�?500 个实体原语�?);
            }
            if (place.Count > 0 && !loadIntoProject)
            {
                throw new BridgeCommandException("create_family.place 需�?load_into_project=true�?);
            }
            if (File.Exists(savePath) && !overwriteFile)
            {
                throw new BridgeCommandException("目标族文件已存在。设�?overwrite_file=true 才会覆盖�? + savePath);
            }
                ...
                if (loadIntoProject)
                {
                    Family loadedFamily;
                        ...
                        try
                        {
                            ...
                            plan["loaded"] = true;
                            plan["family_id"] = (int)loadedFamily.Id.GetValue();
                            plan["family_name"] = loadedFamily.Name;
                            plan["placement_type"] = loadedFamily.FamilyPlacementType.ToString();
                            plan["symbol_ids"] = loadedFamily.GetFamilySymbolIds().Select(id => (int)id.GetValue()).ToArray();
        ...
        private static string ResolveTemplatePath(UIApplication uiApplication, string requestedTemplate)
        {
            if (!string.IsNullOrWhiteSpace(requestedTemplate))
            {
                ...
                {
                    throw new BridgeCommandException("template_path 必须�?.rft 族样板文件�?);
                }
                return normalized;
            }
            ...
            {
                throw new BridgeCommandException(
                    "没有找到默认“公制常规模�?/ Metric Generic Model”族样板。先调用 list_family_templates 查询后显式传 template_path�?);
            }
            return resolved;
        }
        ...
        /// <summary>
        /// 解析族保存路径：默认�?Documents/RevitCommandBridge/Families/ 目录�?
        ...
        private static string ResolveSavePath(string requestedPath, string familyName)
        {
            ...
            {
                throw new BridgeCommandException("save_path 必须�?.rfa 结尾�?);
            }
            return path;
        }
        ...
        private static string NormalizeFamilyPath(string path)
        {
            ...
            {
                throw new BridgeCommandException("family_path 必须�?.rfa 文件�?);
            }
            return normalized;
        }
        ...
        private static string ValidateFamilyName(string name)
        {
            ...
            {
                throw new BridgeCommandException("family_name 包含无效文件名字符，或长度超�?180�?);
            }
            return name;
        }
        ...
        private static FamilyParameter AddFamilyParameter(FamilyManager manager, FamilyParameterSpec spec)
        {
            ...
            if (overload == null)
            {
                throw new BridgeCommandException("当前 Revit API 未找�?ForgeTypeId 族参数接口�?);
            }
        ...
        /// <summary>
        /// 将简写类型名映射�?Revit SpecTypeId ForgeTypeId 实例�?
        ...
        /// <summary>
        /// 通过反射�?Revit DB 程序集解�?ForgeTypeId 静态属�?字段�?
        ...
        /// <summary>
        /// 将简写分组名映射�?Forge GroupTypeId 成员名�?
        ...
        private static bool ReadBooleanValue(object value)
        {
            ...
            throw new BridgeCommandException("布尔参数必须�?true/false �?0/1�? + value);
        }
            ...
            IDictionary<string, object> place)
        {
            ...
            {
                { "type_id", (int)symbol.Id.GetValue() }
            };
        ...
        /// <summary>
        /// 在已载入的族中解析要放置�?FamilySymbol（通过 type_id �?type_name）�?
            ...
            IDictionary<string, object> values)
        {
            ...
            if (rawTypeId != null)
            {
                ...
                if (byId == null || byId.Family == null || byId.Family.Id != family.Id)
                {
