...
二期已新增 **64 个原子操作**（原 40 个）并建立多版本构建管道（`build/version-manifest.json` 驱动，支持 Revit 2020–2027+，`csc.exe` + `dotnet build` 双管道）。

## Q4：工程整体结构是怎样的？

| 目录/文件 | 作用 |
| --- | --- |
| [`src/`](../src/RevitCommandBridgeApp.cs) | Revit 插件 C# 源码（22 个 `.cs`，按年份编译的 DLL，`IExternalApplication` 入口） |
| [`scripts/`](../scripts/revit-mcp-server.mjs) | 传输层：MCP Server、REST 网关、CLI、OpenAI 兼容本地助手 |
| [`build/`](../build/version-manifest.json) | 版本矩阵清单 + 编译属性（`version-manifest.json` 驱动双管道） |
| [`src-net8/`](../src-net8/) | .NET 8 项目族（Revit 2025–2026，`dotnet build` 管道） |
| [`src-net10/`](../src-net10/) | .NET 10 项目族（Revit 2027+，`dotnet build` 管道） |
| [`setup/RevitAIHubSetup.cs`](../setup/RevitAIHubSetup.cs) | 单文件安装器（自动扫描 Revit 2020–2027+、生成适配 DLL、配置 AI 客户端） |
...
1. **安装**：普通用户直接运行 `release/RevitCommandBridgeSetup.exe`；开发者用 `build.ps1 -RevitVersion 2020` + `install-revit.ps1`。安装器自动扫描 Revit 2020–2027+、按年份生成队列目录 `%LOCALAPPDATA%\RevitCommandBridge\<year>`。
...
6. **安全边界**：不接受任意 C#/Python，只接受白名单原子操作（64 个，见 [`AtomicOperations`](../src/PlanCommandExecutor.cs)）；`preview=true` 干跑；`document_title` 匹配、只读文档拒写、1MB 请求上限；REST 仅绑 127.0.0.1。

## 审查结论

架构上"MCP Server（Node，客户端子进程）↔ 文件队列 ↔ Revit 插件（ExternalEvent 主线程）"的分层干净且各司其职；文档与代码一致性高；安全与事务边界处理到位。

二期扩展将原子操作从 40 个提升至 64 个，覆盖建筑、结构、MEP、族、出图与注释全管线，并建立版本清单驱动的多运行时构建体系（Revit 2020–2027+，csc + dotnet 双管道）。各年份适配包仍需在装有对应 Revit 的机器上完成真机回归（VERSION-SUPPORT.md 已标注 [T] 状态）。
...
Revit API 有数千个对象，本桥接**只开放 64 个手工实现的原子操作**（见 [`AtomicOperations`](../src/PlanCommandExecutor.cs) 数组）。插件在执行前会校验：
...
- **为什么不担心"64 个不够用"**：设计哲学是**组合优于穷举**。这 64 个原子操作是按"高频、可组合"挑选的——"24 根不同标高的管道 + 阀门 + 连接 + 写系统编号 + 选中复核"是一个 `execute_plan` 计划，而不是 24 个新命令。幕墙、楼梯、嵌套族等专用对象在 [`ARCHITECTURE.md`](../ARCHITECTURE.md) 的"已知边界"中标注为 [T]（待按需增加原子操作）。
- **每个 Revit 年份要单独编译**：新增原子操作后需为各年份重新编译对应 DLL（API 版本不兼容，不能共用一个 DLL）。构建管道已支持 `build-all.ps1` 批量编译 2020–2027+，`build/version-manifest.json` 驱动双管道（csc/dotnet）。
